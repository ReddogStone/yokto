...
module.exports = function(layers, errorHandler) {
	...
	function handleError(request, error) {
		...
		return Promise.resolve(errorHandler(request, error)).then(finalizer => finalizer || fallback(error));
	}
	...
	return function(request, response) {
		let process = layers.reduceRight(
			(memo, current) =>
				request =>
					Promise.resolve(current(request)).then(finalizer => finalizer || memo(request)),
			handleError
		);
		process(request)
